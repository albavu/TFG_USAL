...
c_out = '/lhome/ext/usal053/usal0531/TFG/ML/OHE'
...
df12 = pd.read_csv('/lhome/ext/usal053/usal0531/TFG/data/NucleocSeq.csv')
df = pd.concat([df, df1], axis=0)
df = df.sample(frac=1).reset_index(drop=True)
df['secuencia'] = df['secuencia'].apply(lambda x: re.sub(r'([A-Z])(?!$)', r'\1,', x))
df = df.reset_index(drop=True)

df12['secuencia'] = df12['secuencia'].apply(lambda x: re.sub(r'([A-Z])(?!$)', r'\1,', x))
df12 = df12.reset_index(drop=True)

df2 = pd.DataFrame(df['secuencia'].str.split(',').values.tolist())
df3 = pd.DataFrame(df12['secuencia'].str.split(',').values.tolist())

df2['nucleosoma'] = 0
df2 = df2.iloc[0:df3.shape[0],:]

df3['nucleosoma'] = 1
total = pd.concat([df2,df3],axis=0)
...
X_train = train.iloc[:,1:736]
y_train = train['nucleosoma']
X_validate = validate.iloc[:,1:736]
y_validate = validate['nucleosoma']
X_test = test.iloc[:,1:736]
...
RF.to_csv(c_out+'/OHE_RF_results.csv')
...
for title, normalize in titles_options:
    disp = plot_confusion_matrix(cf1, X_test, y_test,
                                 display_labels=['No-Nucleosoma','Nucleosoma'],
                                 cmap=plt.cm.Greens,
                                 normalize=normalize)
    disp.ax_.set_title(title)
    plt.savefig(fname=c_out+"/confusionMatrix/OHE_RF_"+title)
    print(title)
...
RG.to_csv(c_out+'/OHE_RG_results.csv')
...
for title, normalize in titles_options:
    disp = plot_confusion_matrix(cf1, X_test, y_test,
                                 ...
                                 normalize=normalize)
    disp.ax_.set_title(title)
    plt.savefig(fname=c_out+"/confusionMatrix/OHE_RG_"+title)
    print(title)
    print(disp.confusion_matrix)
...
NN.to_csv(c_out+'/OHE_KNN_results.csv')
...
for title, normalize in titles_options:
    disp = plot_confusion_matrix(cf1, X_test, y_test,
                                 ...
                                 normalize=normalize)
    disp.ax_.set_title(title)
    plt.savefig(fname=c_out+"/confusionMatrix/OHE_KNN_"+title)
    print(title)
    print(disp.confusion_matrix)
...
DT.to_csv(c_out+'/OHE_DT_results.csv')
...
for title, normalize in titles_options:
    disp = plot_confusion_matrix(cf1, X_test, y_test,
                                 ...
                                 normalize=normalize)
    disp.ax_.set_title(title)
    plt.savefig(fname=c_out+"/confusionMatrix/OHE_DT_"+title)
    print(title)
    print(disp.confusion_matrix)
...
NB.to_csv(c_out+'/OHE_NB_results.csv')
...
for title, normalize in titles_options:
    disp = plot_confusion_matrix(cf1, X_test, y_test,
                                 ...
                                 normalize=normalize)
    disp.ax_.set_title(title)
    plt.savefig(fname=c_out+"/confusionMatrix/OHE_NB_"+title)
    print(title)
    print(disp.confusion_matrix)
